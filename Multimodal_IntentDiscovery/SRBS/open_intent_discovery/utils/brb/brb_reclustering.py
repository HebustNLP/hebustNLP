...
def brb_short_printout(reset_args: Dict[str, Union[int, float, bool]]) -> None:
...
def brb_settings_printout(reset_args: Dict[str, Union[int, float, bool]]) -> None:
...
def soft_reset_only(module, optimizer, reset_interpolation_factor=0.8,
                    reset_batchnorm=False, step=0.0):
    # 复用 soft_reset 的实现
    args = SimpleNamespace(
        reset_weights=True,
        reset_interpolation_factor=reset_interpolation_factor,
        reset_interpolation_factor_step=step,
        reset_batchnorm=reset_batchnorm,
        reset_embedding=True,
        reset_projector=False,
        reset_convlayers=False
    )
    soft_reset(autoencoder=module,
               reset_interpolation_factor=args.reset_interpolation_factor,
               reset_interpolation_factor_step=args.reset_interpolation_factor_step,
               reset_batchnorm=args.reset_batchnorm,
               reset_embedding=args.reset_embedding,
               reset_projector=args.reset_projector,
               reset_convlayers=args.reset_convlayers)
