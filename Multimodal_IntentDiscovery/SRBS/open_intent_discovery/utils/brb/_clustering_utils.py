...
@torch.no_grad()
def encode_batchwise(dataloader, module, device, subsample_size=None):
    embeddings = []
    sample_size_i = 0
    if subsample_size is None:
        subsample_size = np.inf
    for batch in dataloader:
        input_ids, input_mask, segment_ids = batch[0].to(device), batch[1].to(device), batch[2].to(device)
        if sample_size_i < subsample_size:
            emb_i, _ = module(input_ids, segment_ids, input_mask, feature_ext=True)
        else:
            break
        embeddings.append(emb_i.detach().cpu())
        sample_size_i += input_ids.shape[0]
