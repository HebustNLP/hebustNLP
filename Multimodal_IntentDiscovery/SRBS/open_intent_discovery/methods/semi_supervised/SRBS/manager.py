...
from utils.brb.brb_reclustering import apply_brb
...
class SRBSManager:
    ...
    def __init__(self, args, data, model, logger_name = 'Discovery'):
        ...
        pretrain_manager = PretrainSRBSManager(args, data, model)  
        
        set_seed(args.seed)
        self.logger = logging.getLogger(logger_name)

        self.reset_args = SimpleNamespace(
            reset_weights=True,
            recluster=True,
            recalculate_centers=False,
            reset_momentum=True,
            reset_momentum_counts=True,
            reset_interpolation_factor=0.8,
            reset_interpolation_factor_step=0.0,
            reset_batchnorm=False,
            reset_embedding=True,
            reset_projector=False,
            reset_convlayers=False,
            reclustering_method="kmeans",
            subsample_size=None
        )
    ...
    def set_centers(self, new_centers):
        self.centroids = torch.tensor(new_centers).to(self.device)
    ...
    def train(self, args, data): 
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"): 
            ...
            # === Apply BRB ===
            self.model, self.optimizer, _ = apply_brb(
            ...
            if epoch > 0:
                ...
                if delta_label < args.tol and epoch > 35 :
