...
class TCMDataset(Dataset):
    def __init__(self, root_path="./data", image_path="", name="", mode="", seed=2023, k=5, test=0):
        ...
        drop_list = []
        for i, v in self.data['face_number'].items():  # 遍历new_id这一列 i为索引 v为new_id每行的值
        ...
        self.data = self.data.reset_index(drop=True)  # 重置数据集索引   self.data为 419×81
    ...
    def __getitem__(self, index):  # 根据传入的索引dataset[0] 可以获取索引位置的相关信息 比如id ask label等
        sample = {
            'id': self.data['face_number'][index],
        ...
        sample['img'] = self.img[self.data['face_number'][index]]  # 将与该样本ID对应的图片数据从图片数据集中取出,并将其存储在字典sample中的'img'键下
        return sample
...
def main(label_name ="", seed=2023, modality="", path=""):  # 改动
    ...
    fh = logging.FileHandler(Path('./train_log', f"single_face_zxh.log"))
    ...
    model_save_path = f"./model_save/single_face_zxh.pt"
...
#     argument = [("face","/home/zhangxiaohan/code2/constitution_2/data/face/feature/feature_vit/face_before_zxh.pkl"),
#                 ("face_468","/home/zhangxiaohan/code2/constitution_2/data/face/feature/feature_vit_afterseg/face_mediapipe.pkl"),]

if __name__ == "__main__":
    argument = [("face","/home/zhangxiaohan/constitution/tizhi/data/face/feature/feature_after/face_after_zxh.pkl"),
                ("face","/home/zhangxiaohan/constitution/tizhi/data/face/feature/feature_before/face_before_zxh.pkl"),
                ("face","/home/zhangxiaohan/constitution/tizhi/data/face/feature/feature_mediapipe/face_mediapipe_zxh.pkl"),
                ("face","/home/zhangxiaohan/constitution/tizhi/data/face/feature/feature_swin/face_swin_zxh.pkl")]
