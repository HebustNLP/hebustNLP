...
def calculate_probability_differences(model_1, model_2, tokenizer, prompts_1, prompts_2, responses,
                                      batch_size=8, device=None, process_id=None,
                                      num_samples=3, use_variance_weighting=True):
    """

    Args:
        num_samples: 每个输入进行多少次前向传播
        use_variance_weighting: 是否使用方差进行权重调整
    """
    all_weights = []
    all_explain_data = []
    ...
    for i in tqdm(range(0, len(prompts_1), batch_size), desc=desc, mininterval=1.0, ncols=80):
        ...
        padded_input_ids_1 = [F.pad(ids, (0, max_len_1 - len(ids)), value=tokenizer.pad_token_id) for ids in
                              combined_input_ids_1]
        padded_attention_mask_1 = [F.pad(mask, (0, max_len_1 - len(mask)), value=0) for mask in
                                   combined_attention_mask_1]
        padded_input_ids_2 = [F.pad(ids, (0, max_len_2 - len(ids)), value=tokenizer.pad_token_id) for ids in
                              combined_input_ids_2]
        padded_attention_mask_2 = [F.pad(mask, (0, max_len_2 - len(mask)), value=0) for mask in
                                   combined_attention_mask_2]
        ...
        for sample_idx in range(num_samples):
            with torch.no_grad():
                ...
                if sample_idx > 0:
                    # 添加极小的噪声来模拟数值计算的不确定性
                    noise_scale = 1e-5
                    noise_1 = torch.randn_like(inputs_1['input_ids'], dtype=torch.float) * noise_scale
                    noise_2 = torch.randn_like(inputs_2['input_ids'], dtype=torch.float) * noise_scale
                else:
                    noise_1 = 0
                    noise_2 = 0
                ...
                all_logits_1.append(log_probs_1.cpu().numpy())
                all_logits_2.append(log_probs_2.cpu().numpy())

        # Calculate probability differences with variance
        batch_weights = []
        batch_explain_data = []

        for j in range(len(batch_prompts_1)):
            prompt_length_1 = len(tokenizer.encode(batch_prompts_1[j])) - 1
            prompt_length_2 = len(tokenizer.encode(batch_prompts_2[j])) - 1
            ...
            for k in range(response_length):
                ...
                weight_samples = []
                for sample_idx in range(num_samples):
                    score_1 = all_logits_1[sample_idx][j, prompt_length_1 + k, actual_next_token_id_1]
                    score_2 = all_logits_2[sample_idx][j, prompt_length_2 + k, actual_next_token_id_2]
                    weight_samples.append(score_2 - score_1)

                # 计算均值和方差
                weight_samples = np.array(weight_samples)
                mean_weight = np.mean(weight_samples)
                std_weight = np.std(weight_samples)

                # ===== 方差加权策略 =====
                if use_variance_weighting and std_weight > 0:

                    shrinkage_factor = 1.0 / (1.0 + std_weight ** 2)
                    adjusted_weight = mean_weight * shrinkage_factor

                    final_weight = adjusted_weight
                else:
                    final_weight = mean_weight

                weights.append(round(float(final_weight), 2))

                # 可选：记录详细信息用于分析
                if k < 3:  # 只记录前几个token的详细信息
                    explain_data.append({
                        'token': tokenizer.decode([actual_next_token_id_1]),
                        'position': k,
                        'mean_weight': round(float(mean_weight), 3),
                        'std_weight': round(float(std_weight), 3),
                        'final_weight': round(float(final_weight), 3),
                        'samples': [round(float(w), 3) for w in weight_samples]
                    })

            assert len(weights) == response_length
            batch_weights.append(weights)
            batch_explain_data.append(explain_data)

        all_weights.extend(batch_weights)
        all_explain_data.extend(batch_explain_data)
...
def process_dataset_shard(gpu_id, input_file, model_name_1, model_name_2, model1_template, model2_template, data_shard,
                          batch_size=8):
    ...
    # 使用改进的函数，启用多次采样和方差加权
    rejected_weights, rejected_explain = calculate_probability_differences(
        model_1, model_2, tokenizer, prompts1, prompts2, rejected_responses,
        batch_size=batch_size, device=device, process_id=gpu_id,
        num_samples=5, use_variance_weighting=True
    )
    chosen_weights, chosen_explain = calculate_probability_differences(
    ...
    for i, item in enumerate(data_shard):
        ...
        # 可选：添加解释数据的摘要
        if i < 3 and rejected_explain[i]:  # 只为前几个样本添加
            item['weight_explain_sample'] = {
                'rejected_first_tokens': rejected_explain[i][:3],
                'chosen_first_tokens': chosen_explain[i][:3] if i < len(chosen_explain) else []
            }
